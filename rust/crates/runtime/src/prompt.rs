...
fn discover_instruction_files(cwd: &Path) -> std::io::Result<Vec<ContextFile>> {
    ...
    for dir in directories {
        for candidate in [
            dir.join("RUNE.md"),
            dir.join("RUNE.local.md"),
            dir.join(".rune").join("RUNE.md"),
            dir.join(".rune").join("instructions.md"),
        ] {
...
fn render_config_section(config: &RuntimeConfig) -> String {
    ...
    if config.loaded_entries().is_empty() {
        lines.extend(prepend_bullets(vec![
            "No Rune settings files loaded.".to_string()
        ]));
        return lines.join("\n");
    }
...
mod tests {
    ...
    fn discovers_instruction_files_from_ancestor_chain() {
        ...
        fs::create_dir_all(nested.join(".rune")).expect("nested rune dir");
        fs::write(root.join("RUNE.md"), "root instructions").expect("write root instructions");
        fs::write(root.join("RUNE.local.md"), "local instructions")
            .expect("write local instructions");
        fs::create_dir_all(root.join("apps")).expect("apps dir");
        fs::create_dir_all(root.join("apps").join(".rune")).expect("apps rune dir");
        fs::write(root.join("apps").join("RUNE.md"), "apps instructions")
            .expect("write apps instructions");
        fs::write(
            root.join("apps").join(".rune").join("instructions.md"),
            "apps dot claude instructions",
        )
        .expect("write apps dot claude instructions");
        fs::write(nested.join(".rune").join("RUNE.md"), "nested rules")
            .expect("write nested rules");
        fs::write(
            nested.join(".rune").join("instructions.md"),
            "nested instructions",
        )
    ...
    fn dedupes_identical_instruction_content_across_scopes() {
        ...
        fs::write(root.join("RUNE.md"), "same rules\n\n").expect("write root");
        fs::write(nested.join("RUNE.md"), "same rules\n").expect("write nested");
    ...
    fn displays_context_paths_compactly() {
        assert_eq!(
            display_context_path(Path::new("/tmp/project/.rune/RUNE.md")),
            "RUNE.md"
        );
    }
    ...
    fn discover_with_git_includes_status_snapshot() {
        ...
        fs::write(root.join("RUNE.md"), "rules").expect("write instructions");
        ...
        assert!(status.contains("?? RUNE.md"));
    ...
    fn load_system_prompt_reads_claude_files_and_config() {
        ...
        fs::write(root.join("RUNE.md"), "Project rules").expect("write instructions");
        fs::write(
            root.join(".rune").join("settings.json"),
...
",
    ...
    fn discovers_dot_claude_instructions_markdown() {
        ...
        assert!(context
            .instruction_files
            .iter()
            .any(|file| file.path.ends_with(".rune/instructions.md")));
        assert!(
    ...
    fn renders_instruction_file_metadata() {
        let rendered = render_instruction_files(&[ContextFile {
            path: PathBuf::from("/tmp/project/RUNE.md"),
