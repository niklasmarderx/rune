...
pub const RUNE_SETTINGS_SCHEMA_NAME: &str = "SettingsSchema";
...
/// OAuth client configuration used by the main Rune runtime.
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl ConfigLoader {
    ...
    pub fn discover(&self) -> Vec<ConfigEntry> {
        let user_legacy_path = self.config_home.parent().map_or_else(
            || PathBuf::from(".rune.json"),
            |parent| parent.join(".rune.json"),
        );
        vec![
            ConfigEntry {
            ...
            ConfigEntry {
                ...
                path: self.cwd.join(".rune.json"),
            },
            ConfigEntry {
                source: ConfigSource::Project,
                path: self.cwd.join(".rune").join("settings.json"),
            },
            ConfigEntry {
                source: ConfigSource::Local,
                path: self.cwd.join(".rune").join("settings.local.json"),
            },
        ]
    }
...
pub fn default_config_home() -> PathBuf {
    std::env::var_os("RUNE_CONFIG_HOME")
        .map(PathBuf::from)
        .or_else(|| std::env::var_os("HOME").map(|home| PathBuf::from(home).join(".rune")))
        .unwrap_or_else(|| PathBuf::from(".rune"))
}
...
fn read_optional_json_object(
    ...
    let is_legacy_config = path.file_name().and_then(|name| name.to_str()) == Some(".rune.json");
...
mod tests {
    use super::{
        ...
        RuntimePluginConfig, RUNE_SETTINGS_SCHEMA_NAME,
    };
    use crate::json::JsonValue;
    ...
    fn rejects_non_object_settings_files() {
        ...
        let home = root.join("home").join(".rune");
    ...
    #[test]
    fn loads_and_merges_rune_config_files_by_precedence() {
        let root = temp_dir();
        let cwd = root.join("project");
        let home = root.join("home").join(".rune");
        fs::create_dir_all(cwd.join(".rune")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");

        fs::write(
            home.parent().expect("home parent").join(".rune.json"),
        ...
        assert_eq!(RUNE_SETTINGS_SCHEMA_NAME, "SettingsSchema");
    ...
    fn rejects_invalid_hook_entries_before_merge() {
        ...
        let cwd = root.join("project");
        let home = root.join("home").join(".rune");
        let project_settings = cwd.join(".rune").join("settings.json");
        fs::create_dir_all(cwd.join(".rune")).expect("project config dir");
        fs::create_dir_all(&home).expect("home config dir");
