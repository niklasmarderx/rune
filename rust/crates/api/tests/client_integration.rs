...
/// Cross-process lock that serialises all env-mutating tests across test
/// binaries.  Each binary also uses an in-process mutex so concurrent test
/// threads within the same binary are serialised too.
...
async fn send_message_applies_request_profile_and_records_telemetry() {
    ...
    let client = AnthropicClient::new("test-key")
        .with_base_url(server.base_url())
        .with_client_identity(ClientIdentity::new("rune-code", "9.9.9").with_runtime("rust-cli"))
        .with_beta("tools-2026-04-01")
        .with_extra_body_param("metadata", json!({"source": "rune"}))
    ...
    assert_eq!(
        request.headers.get("user-agent").map(String::as_str),
        Some("rune-code/9.9.9")
    );
    let body: serde_json::Value =
        serde_json::from_str(&request.body).expect("request body should be json");
    assert_eq!(body["metadata"]["source"], json!("rune"));
    assert_eq!(
        body["betas"],
        json!([
...
async fn stream_message_parses_sse_events_with_tool_use() {
    ...
    std::env::set_var("RUNE_CONFIG_HOME", &temp_root);
    let state = Arc::new(Mutex::new(Vec::<CapturedRequest>::new()));
    let sse = concat!(
        "event: message_start\n",
