...
const MANIFEST_RELATIVE_PATH: &str = ".rune-plugin/plugin.json";
...
impl PluginTool {
    ...
    pub fn execute(&self, input: &Value) -> Result<String, PluginError> {
        ...
        process
            ...
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
            .env("RUNE_PLUGIN_ID", &self.plugin_id)
            .env("RUNE_PLUGIN_NAME", &self.plugin_name)
            .env("RUNE_TOOL_NAME", &self.definition.name)
            .env("RUNE_TOOL_INPUT", &input_json);
        if let Some(root) = &self.root {
            process
                .current_dir(root)
                .env("RUNE_PLUGIN_ROOT", root.display().to_string());
        }
