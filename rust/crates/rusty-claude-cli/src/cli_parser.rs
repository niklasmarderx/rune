...
use super::{
    build_runtime_plugin_state_with_loader, DEFAULT_DATE, DEFAULT_MODEL, LATEST_SESSION_REFERENCE,
};
...
pub(crate) fn parse_args(args: &[String]) -> Result<CliAction, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            ...
            "-p" => {
                // Rune Code compat: -p "prompt" = one-shot prompt
            ...
            "--print" => {
                // Rune Code compat: --print makes output non-interactive
...
fn bare_slash_command_guidance(command_name: &str) -> Option<String> {
    ...
    let guidance = if slash_command.resume_supported {
        format!(
            "`rune {command_name}` is a slash command. Use `rune --resume SESSION.jsonl /{command_name}` or start `rune` and run `/{command_name}`."
        )
    } else {
        format!(
            "`rune {command_name}` is a slash command. Start `rune` and run `/{command_name}` inside the REPL."
        )
    };
    Some(guidance)
}
...
fn parse_direct_slash_cli_action(rest: &[String]) -> Result<CliAction, String> {
    ...
    match SlashCommand::parse(&raw) {
        ...
        Ok(Some(command)) => Err({
            let _ = command;
            format!(
                "slash command {command_name} is interactive-only. Start `rune` and run it there, or use `rune --resume SESSION.jsonl {command_name}` / `rune --resume {latest} {command_name}` when the command is marked [resume] in /help.",
                command_name = rest[0],
...
fn format_unknown_direct_slash_command(name: &str) -> String {
    ...
    message.push_str("\nRun `rune --help` for CLI usage, or start `rune` and use /help.");
    message
}
