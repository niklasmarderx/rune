...
const GITIGNORE_COMMENT: &str = "# Rune local artifacts";
const GITIGNORE_ENTRIES: [&str; 2] = [".rune/settings.local.json", ".rune/sessions/"];
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let claude_dir = cwd.join(".rune");
    artifacts.push(InitArtifact {
        name: ".rune/",
        status: ensure_dir(&claude_dir)?,
    });

    let claude_json = cwd.join(".rune.json");
    artifacts.push(InitArtifact {
        name: ".rune.json",
    ...
    let claude_md = cwd.join("RUNE.md");
...
pub(crate) fn render_init_claude_md(cwd: &Path) -> String {
    ...
    let mut lines = vec![
        "# RUNE.md".to_string(),
        String::new(),
        "This file provides guidance to Rune when working with code in this repository."
            .to_string(),
        String::new(),
    ];
    ...
    lines.push("- Keep shared defaults in `.rune.json`; reserve `.rune/settings.local.json` for machine-local overrides.".to_string());
    lines.push("- Do not overwrite existing `RUNE.md` content automatically; update it intentionally when repo workflows change.".to_string());
    lines.push(String::new());
...
mod tests {
    ...
    fn temp_dir() -> std::path::PathBuf {
        let nanos = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("rune-init-{nanos}"))
    }
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".rune/           created"));
        assert!(rendered.contains(".rune.json       created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("RUNE.md          created"));
        assert!(root.join(".rune").is_dir());
        assert!(root.join(".rune.json").is_file());
        assert!(root.join("RUNE.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".rune.json")).expect("read claude json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        assert!(gitignore.contains(".rune/settings.local.json"));
        assert!(gitignore.contains(".rune/sessions/"));
        let claude_md = fs::read_to_string(root.join("RUNE.md")).expect("read claude md");
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join("RUNE.md"), "custom guidance\n").expect("write existing claude md");
        fs::write(root.join(".gitignore"), ".rune/settings.local.json\n").expect("write gitignore");

        let first = initialize_repo(&root).expect("first init should succeed");
        assert!(first
            .render()
            .contains("RUNE.md          skipped (already exists)"));
        let second = initialize_repo(&root).expect("second init should succeed");
        let second_rendered = second.render();
        assert!(second_rendered.contains(".rune/           skipped (already exists)"));
        assert!(second_rendered.contains(".rune.json       skipped (already exists)"));
        assert!(second_rendered.contains(".gitignore       skipped (already exists)"));
        assert!(second_rendered.contains("RUNE.md          skipped (already exists)"));
        assert_eq!(
            fs::read_to_string(root.join("RUNE.md")).expect("read existing claude md"),
            "custom guidance\n"
        );
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".rune/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".rune/sessions/").count(), 1);
