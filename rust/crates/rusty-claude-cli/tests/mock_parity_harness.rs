...
#[test]
#[ignore = "requires mock server fix: CLI sends 401 to real API instead of mock (pre-existing issue)"]
...
fn run_case(case: ScenarioCase, workspace: &HarnessWorkspace, base_url: &str) -> ScenarioRun {
    ...
    command
        ...
        .env("RUNE_CONFIG_HOME", &workspace.config_home)
    ...
    let output = if let Some(stdin) = case.stdin {
        ...
        child.wait_with_output().expect("rune should finish")
    } else {
...
fn prepare_auto_compact_fixture(workspace: &HarnessWorkspace) {
    let sessions_dir = workspace.root.join(".rune").join("sessions");
...
fn prepare_plugin_fixture(workspace: &HarnessWorkspace) {
    ...
    fs::write(
        &script_path,
        "#!/bin/sh\nINPUT=$(cat)\nprintf '{\"plugin\":\"%s\",\"tool\":\"%s\",\"input\":%s}\\n' \"$RUNE_PLUGIN_ID\" \"$RUNE_TOOL_NAME\" \"$INPUT\"\n",
    )
...
fn unique_temp_dir(label: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!(
        "rune-mock-parity-{label}-{}-{millis}-{counter}",
        std::process::id()
    ))
}
